...
void GeneratorTest2(void)
{
    ...
    for (int Depth = 1; Depth <= MaxDepth; ++Depth) {
        ...
        printf("Depth = %2d Result nodes = %10lu\n", Depth, ResultNodes);
    }
}
...
void SearchPerformanceTest(void)
{
    ...
    U64 TotalTestNodes = 0UL;
    U64 TotalTestTime = 0UL;
    ...
    printf("Total nodes %lu Total time %.2f\n", TotalTestNodes, (double)TotalTestTime / 1000.0);

    printf("Average nodes %lu Average time %.2f\n", TotalTestNodes / (U64)MaxCycles, (double)TotalTestTime / (double)MaxCycles / 1000.0);

    if (TotalTestTime >= 1000UL) {
        printf("NPS %lu\n", 1000UL * TotalTestNodes / TotalTestTime);
    }
}
...
void EvaluateTests(const char* Tests[], const int TestCount)
{
    ...
    U64 EvaluateTimeStart;
    U64 EvaluateTimeStop;

    U64 EvaluateTotalTime = 0UL;
    ...
    for (int Cycle = 0; Cycle < MaxCycles; ++Cycle) {
        ...
        for (int TestNumber = 0; TestNumber < TestCount; ++TestNumber) {
            ...
            EvaluateTimeStart = ClockNS();

            Evaluate(&CurrentBoard);

            EvaluateTimeStop = ClockNS();

            EvaluateTotalTime += EvaluateTimeStop - EvaluateTimeStart;
        }
    }

    printf("\n");

    printf("EPS %lu\n", 1000000000UL * (U64)TestCount * (U64)MaxCycles / EvaluateTotalTime);
}
