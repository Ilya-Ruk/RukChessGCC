...
I16 InputWeights[INPUT_DIMENSION * HIDDEN_DIMENSION] __attribute__ ((aligned (64)));    // 768 x 256 = 196608
I16 InputBiases[HIDDEN_DIMENSION] __attribute__ ((aligned (64)));                       // 256
I16 OutputWeights[HIDDEN_DIMENSION * 2] __attribute__ ((aligned (64)));                 // 256 x 2 = 512
I32 OutputBias;                                                                         // 1
...
//    printf("File position = %lu\n", FilePos);
    ...
    printf("Load network...DONE (%s; 0x%012lx)\n", NnueFileName, FileHash);
...
void RefreshAccumulator(BoardItem* Board)
{
    ...
    for (int Perspective = 0; Perspective < 2; ++Perspective) { // White/Black
        ...
        while (Pieces) {
            ...
            for (int Reg = 0; Reg < (int)NUM_REGS; ++Reg) { // 16
