...
{
    return __builtin_popcountll(Source);
}

int LSB(const U64 Source)
{
    return __builtin_ctzll(Source);
}

int MSB(const U64 Source)
{
    return __builtin_clzll(Source);
}
...
void InitBitBoards(void)
{
    ...
    for (int Square = 0; Square < 64; ++Square) {
        BB_BishopMask[Square] = CalculatePieceAttack(BISHOP, Square, 0UL, TRUE);
        BB_RookMask[Square] = CalculatePieceAttack(ROOK, Square, 0UL, TRUE);
    }

    // Init Knight and King attack tables; Bishop and Rook offset and attack tables
    for (int Square = 0; Square < 64; ++Square) {
        BB_KnightAttack[Square] = CalculatePieceAttack(KNIGHT, Square, 0UL, FALSE);
        BB_KingAttack[Square] = CalculatePieceAttack(KING, Square, 0UL, FALSE);
