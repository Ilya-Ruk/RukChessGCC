...
{
    U64 CurrentTime = ClockMS();

    TotalTime = CurrentTime - TimeStart;

    if (PrintMode == PRINT_MODE_UCI) {
        printf("info depth %d seldepth %d nodes %lu time %lu", Depth, Board->SelDepth, Board->Nodes, TotalTime);
        ...
        if (TotalTime >= 1000UL) {
            printf(" nps %lu", 1000UL * Board->Nodes / TotalTime);
        }

        if ((TimeStop - CurrentTime) >= 3000UL) {
        ...
        printf("Depth %2d / %2d Score %6.2f Nodes %9lu Hashfull %5.2f%% Time %6.2f", Depth, Board->SelDepth, (double)BestScore / 100.0, Board->Nodes, (double)FullHash() / 10.0, (double)TotalTime / 1000.0);
...
BOOL PrintResult(const BOOL InCheck, const MoveItem BestMove, const MoveItem PonderMove, const int BestScore)
{
    ...
    if (PrintMode == PRINT_MODE_UCI) {
        printf("info nodes %lu", CurrentBoard.Nodes);

        if (TotalTime >= 1000UL) {
            printf(" nps %lu", 1000UL * CurrentBoard.Nodes / TotalTime);
        }

        if ((TimeStop - ClockMS()) >= 3000UL) {
            printf(" hashfull %d", FullHash());
        }
        ...
        printf("Score %.2f Nodes %lu Hashfull %.2f%% Time %.2f", (double)BestScore / 100.0, CurrentBoard.Nodes, (double)FullHash() / 10.0, (double)TotalTime / 1000.0);
...
BOOL ComputerMove(void)
{
    ...
    CurrentBoard.Nodes = 0UL;

#ifdef USE_STATISTIC
    CurrentBoard.HashCount = 0UL;
    CurrentBoard.EvaluateCount = 0UL;
    CurrentBoard.CutoffCount = 0UL;
    CurrentBoard.QuiescenceCount = 0UL;
#endif // USE_STATISTIC
...
*/
            ThreadBoard.Nodes = 0UL;

#ifdef USE_STATISTIC
            ThreadBoard.HashCount = 0UL;
            ThreadBoard.EvaluateCount = 0UL;
            ThreadBoard.CutoffCount = 0UL;
            ThreadBoard.QuiescenceCount = 0UL;
#endif // USE_STATISTIC
...
#pragma omp critical
                ...
                if (TargetTimeLocal > 0UL && BestScore > ThreadScore) {
                    TargetTimeLocal = (U64)((double)TargetTimeLocal * MIN((1.0 + (double)(BestScore - ThreadScore) / 80.0), 2.0));
                }

                BestScore = ThreadScore;

                if (TargetTimeLocal > 0UL && CompletedDepth >= MIN_SEARCH_DEPTH && (ClockMS() - TimeStart) >= TargetTimeLocal) { // Time is up
                    break; // for (depth)
                }
            }
